...
def findall_items(xml: str):
    # Devuelve lista con el XML crudo de cada <item>…</item>
...
def find_attr(xml: str, tag: str, attr: str):
    # Busca atributos tipo <tag ... attr="valor" ... />
...
def replace_description(item_xml: str, new_desc_html_cdata: str) -> str:
    # Si hay <description>…</description> lo sustituimos; si no, lo insertamos antes de </item>
...
def has_rich_html(text: str) -> bool:
    if not text:
        return False
    return re.search(r"<(a|img|ul|ol|li|h[1-6])\b", text, flags=re.IGNORECASE) is not None
...
def process_description_block(title_txt: str, link_txt: str, image_url: str, description_inner: str) -> str:
    ...
    Devuelve CDATA con:
    <h3>título</h3>
    <a href="link"><img src="image" /></a> (si hay imagen)
    <hr style="border:0;border-top:1px dashed #ccc;margin:20px 0;" />
    + cuerpo (si ya hay HTML fuerte, no lo tocamos; si no, solo transformaciones mínimas)
    """
    header = ""
    if title_txt:
    ...
    # Mantener EXACTAMENTE tu hr, sin espacios añadidos
    ...
    if has_rich_html(body):
        # Ya viene con HTML enriquecido → no tocar
        final_html = header + body
        return enc_cdata(final_html)

    # Caso "texto plano": aplicar transformaciones suaves, sin beautifiers.
    # 1) Emails → mailto:
    body = re.sub(
    ...
    # 4) Párrafos básicos por líneas (sin embellecido)
...
def existing_keys_from_feed(xml: str) -> set:
    keys = set()
    for it in findall_items(xml):
        keys.add(item_key_from_xml(it))
    return keys
...
def update_feed_dir(feed_dir: str):
    ...
    # Acumular ítems nuevos en una lista (luego los pondremos ARRIBA)
    new_items = []
    ...
    for url in source_urls:
        try:
            for raw_item in fetch_source_items(url):
                ...
                # Extraer datos mínimos para construir la nueva <description>
                ...
                # Reemplazar SOLO la description del item crudo
    ...
    # Queremos los nuevos ARRIBA → los ordenamos tal cual vienen (normalmente ya en desc)
    # y los insertamos antes del primer <item>. Si no hay, antes de </channel>.
    insertion_block = "\n".join(new_items)
