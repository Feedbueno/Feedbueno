...
def update_feed_dir_imetal(feed_dir: str):
    source_file = os.path.join(feed_dir, "imetal.txt")
    dest_file = os.path.join(feed_dir, "feed.xml")

    if not (os.path.exists(source_file) and os.path.exists(dest_file)):
        print(f"⏭️  Omitido {feed_dir}: falta imetal.txt o feed.xml")
        return
    ...
    if not source_urls:
        print(f"ℹ️  {feed_dir}: imetal.txt vacío")
        return
    ...
    for url in source_urls:
        try:
            for raw_item in fetch_source_items(url):
                ...
                if "Iniciativa Metal Gear" not in strip_cdata(title_inner):
                    continue  # Filtrar solo los títulos deseados
                ...
                new_item = ensure_itunes_tags(new_item, strip_cdata(title_inner))
    ...
    if not new_items:
        print(f"= {feed_dir}: sin nuevos episodios (Iniciativa Metal Gear)")
        return
    ...
    print(f"✅ {feed_dir}: añadidos {len(new_items)} episodios nuevos (Iniciativa Metal Gear)")
